...
unsigned short num_editors;

void *pop_list(void *pop_s) {	
	ifstream words;
	string dodo;
	long code = (long) pop_s->t9_code;
	llist_t *current, *head = pop_s->head;
...
void *braint9(void *args) {
	...
	cout << "BRAIN JUST STARTED --version t9_list\n";
	...
	llist_t *curr, *curr_dict, *poss_curr, *possible;
		...
		switch(nump) {
			...
			default:
				...
				if(len==0) {
					...
					string dummy;
					Population_type *pop_s;
					pop_s->t9_code = nump;
					pop_s->node = curr_dict;
					pthread_create(&dict_pth, NULL, pop_vec, (void *) pop_s);
